...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const fetchVariants = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/products/${itemCode}/variants?page=${variantPage}`)
        ...
        if (response.ok) {
          ...
          // Check if we need to load more variants
          if (data.totalVariants > 1200 && !data.hasMore) {
            setAllVariantsLoaded(true)
          } else if (data.totalVariants <= 1200) {
            setAllVariantsLoaded(true)
          }
        }
      } catch (err) {
    ...
    fetchVariants()
  }, [product, itemCode, variantPage])

  // Load more variants if needed for filtering
  const loadMoreVariants = async () => {
    if (!allVariantsLoaded && !variantsLoading) {
      setVariantPage((prev) => prev + 1)
    }
  }
  ...
  useEffect(() => {
    ...
    // If we have filters and not all variants are loaded, load more
    if (Object.keys(filters).length > 0 && !allVariantsLoaded && totalVariants > 1200) {
      loadMoreVariants()
    }
    ...
    setCurrentPage(1)
  }, [filters, variants, product, attributeDetails, allVariantsLoaded])
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="lg:w-1/2 p-8">
              ...
              <div className="space-y-3 mb-6">
                ...
                {!allVariantsLoaded && totalVariants > 1200 && (
                  <p className="text-amber-600 dark:text-amber-400 text-sm">
                    <span className="font-medium">Note:</span> Loading variants for filtering... ({variants.length}/
                    {totalVariants})
                  </p>
                )}
              </div>
